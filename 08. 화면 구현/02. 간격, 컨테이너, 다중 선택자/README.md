...
- **요소의 상태가 바뀌는 순간**을 선택하는 의사 클래스다
- 콜론(`:`)으로 시작한다.

| 의사 클래스 | 설명 |
| --- | --- |
| `:link` | 방문하지 않은 링크 상태 |
| `:visited` | 이미 방문한 링크 상태 |
| `:hover` | 마우스를 올렸을 때 |
| `:active` | 클릭 중일 때 |
| `:focus` | 입력창 등에서 포커스를 얻었을 때 |


### 상태 의사 클래스 (UI Element States Pseudo-Classes)

- **폼(form) 요소의 상태**를 선택하는 의사 클래스다.
- 체크박스나 버튼, 입력창의 **활성/비활성/체크 여부** 에 따라 스타일을 다르게 지정한다.
...
- 요소의 **위치나 구조적인 순서**에 따라 선택할 수 있는 의사 클래스다.
- 부모-자식 관계 안에서 특정 순번이나 타입을 선택할 때 사용한다.

| 의사 클래스 | 설명 |
| --- | --- |
| `:first-child` | 첫 번째 자식 요소 선택 |
| `:nth-child(n)` | n번째 자식 요소 선택 |
| `:first-of-type` | 같은 태그 중 첫 번째 선택 |
| `:nth-of-type(n)` | 같은 태그 중 n번째 선택 |


### 기타 의사 클래스 (Others)

- 특정 조건이나 언어 설정 등에 따라 선택할 수 있는 기타 의사 클래스다

| 의사 클래스 | 설명 |
| --- | --- |
| `:not(selector)` | 괄호 안의 선택자를 제외한 나머지 선택 |
| `:lang(language)` | 지정된 언어 속성을 가진 요소 선택 |
