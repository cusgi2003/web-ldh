...
public class MiddleCalculatorExample {
	public static void main(String[] args) {
		...
		// 내림차순
		Arrays.stream(arr).boxed() // 기본형 객체 스트림으로 변경 IntStream -> Stream<Integer>
				.sorted((a, b) -> b - a).forEach(x -> System.out.print(x + " "));
		System.out.println();
		...
		// 글자수 순으로 오름차순
		sList.stream().sorted((a, b) -> a.length() - b.length()).forEach(x -> System.out.print(x + " "));
		System.out.println();
		...
		Arrays.stream(iArr).distinct().forEach(x -> System.out.print(x + " "));

		// 객체의 eqauls()와 hashCode()기준으로 중복을 제거
		...
		// 앞에서부터 지정한 개수만 남긴다
		System.out.println();
		IntStream.rangeClosed(1, 10).limit(3).forEach(x -> System.out.print(x + " "));

		// Skip(long maxSize)
		// 앞에서부터 지정한 개수만 남긴다
		System.out.println();
		IntStream.rangeClosed(1, 10).skip(5).forEach(x -> System.out.print(x + " "));
		...
		.peek(s -> System.out.println("변경 후 : " + s))
