...
public class Main {
	public static void main(String[] args) {
		...
		// 리스트에 들어있는 모든 내용을 출력하는 람다식을 이용해 출력해보기
		for (String fruit : fruits) {
			printFruit.accept(fruit);
		}

		// 출력, 저장, 로그남기기, 리스트 요소 하나씩 처리하기
		...
		// Predicate<T>
		// 값을 받아서 조건을 검사한 뒤 true/false를 반호나하는 인터페이스
		// boolean test(T t);
		...
		members.add(new Member("김민수", 30));
		members.add(new Member("박지은", 22));
		
		// 20세 이상인지 검증하는 람다식
		Predicate<Member> adult = n -> n.age >= 20;
