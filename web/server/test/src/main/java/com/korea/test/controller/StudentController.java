...
public class StudentController {
	...
	public ResponseEntity<?> getStudents() {
		try {
			...
			ResponseDTO<StudentDTO> response = ResponseDTO.<StudentDTO>builder().error(null).data(students).build();
			return ResponseEntity.ok().body(response);
		} catch (Exception e) {
			ResponseDTO<StudentDTO> response = ResponseDTO.<StudentDTO>builder().error("수강생 목록 조회에 실패했습니다.").build();
			return ResponseEntity.ok().body(response);
		}
